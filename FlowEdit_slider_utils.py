"""
FlowEdit-Slider: 3プロンプト方向性分解による連続スケール制御

FlowEditの実画像編集能力を維持しながら、FreeSlidersの考え方を取り入れて
連続的な編集強度制御を可能にする手法。

数式:
    V_direction = V_tar_pos - V_tar_neg  (純粋な編集方向)
    V_base = V_tar_neg - V_src           (ベース変化)
    V_delta = V_base + scale * V_direction

scale=0: tar_neg方向への編集（例：劣化なし）
scale=1: tar_pos方向への編集（例：完全劣化）
0<scale<1: 連続的な中間状態
"""
...
def compute_vector_stats(
    ...
    prev_V_dir: Optional[torch.Tensor] = None,
    ...
    Args:
        V_base: Base velocity (V_neg - V_src)
        V_dir: Direction velocity (V_pos - V_neg)
        V_delta: Combined velocity (V_base + scale * V_dir)
        zt_edit: Current edited latent
        prev_V_dir: V_dir from previous timestep (for cosine similarity)
    ...
    stats["V_base_norm"] = V_base.norm(dim=-1).mean().item()
    stats["V_dir_norm"] = V_dir.norm(dim=-1).mean().item()
    stats["V_delta_norm"] = V_delta.norm(dim=-1).mean().item()
    stats["zt_edit_norm"] = zt_edit.norm(dim=-1).mean().item()

    # Compute cosine similarity with previous V_dir
    if prev_V_dir is not None:
        # Flatten for cosine similarity computation
        v_dir_flat = V_dir.view(-1)
        prev_v_dir_flat = prev_V_dir.view(-1)
        cos_sim = F.cosine_similarity(v_dir_flat.unsqueeze(0), prev_v_dir_flat.unsqueeze(0)).item()
        stats["V_dir_cosine"] = cos_sim
    else:
        stats["V_dir_cosine"] = 1.0  # First step, no previous

    # Compute angle between V_base and V_dir
    v_base_flat = V_base.view(-1)
    v_dir_flat = V_dir.view(-1)
    cos_angle = F.cosine_similarity(v_base_flat.unsqueeze(0), v_dir_flat.unsqueeze(0)).item()
    # Clamp to avoid numerical issues with arccos
    cos_angle = max(-1.0, min(1.0, cos_angle))
    angle_rad = np.arccos(cos_angle)
    stats["V_base_V_dir_angle"] = np.degrees(angle_rad)
...
def save_vector_stats(
    ...
    output_data = {
        ...
        "V_base_norm": [s["V_base_norm"] for s in stats_list],
        "V_dir_norm": [s["V_dir_norm"] for s in stats_list],
        "V_delta_norm": [s["V_delta_norm"] for s in stats_list],
        "V_dir_cosine": [s["V_dir_cosine"] for s in stats_list],
        "V_base_V_dir_angle": [s["V_base_V_dir_angle"] for s in stats_list],
        "zt_edit_norm": [s["zt_edit_norm"] for s in stats_list],
        "zt_edit_delta": [s["zt_edit_delta"] for s in stats_list],
    }

    output_path = os.path.join(output_dir, f"stats_scale_{scale:.2f}.json")
...
def plot_vector_stats(
    ...
    ax.plot(timesteps, data["V_base_norm"], 'b-', label="V_base", linewidth=2)
    ax.plot(timesteps, data["V_dir_norm"], 'r-', label="V_dir", linewidth=2)
    ax.plot(timesteps, data["V_delta_norm"], 'g--', label="V_delta", linewidth=2)
    ax.set_xlabel("Timestep", fontsize=12)
    ax.set_ylabel("L2 Norm", fontsize=12)
    ax.set_title(f"Velocity Field Norms (scale={scale:.2f})", fontsize=14)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    ax.invert_xaxis()  # t goes from 1.0 to 0
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"plot_norms_scale_{scale:.2f}.png"), dpi=150)
    plt.close()

    # Plot 2: V_dir Cosine Similarity
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(timesteps[1:], data["V_dir_cosine"][1:], 'purple', linewidth=2)
    ax.axhline(y=0.9, color='gray', linestyle='--', alpha=0.7, label="Stability threshold (0.9)")
    ax.set_xlabel("Timestep", fontsize=12)
    ax.set_ylabel("Cosine Similarity", fontsize=12)
    ax.set_title(f"V_dir Directional Consistency (scale={scale:.2f})", fontsize=14)
    ax.set_ylim(-0.1, 1.1)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    ax.invert_xaxis()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"plot_cosine_scale_{scale:.2f}.png"), dpi=150)
    plt.close()

    # Plot 3: Angle between V_base and V_dir
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(timesteps, data["V_base_V_dir_angle"], 'orange', linewidth=2)
    ax.axhline(y=90, color='gray', linestyle='--', alpha=0.7, label="Orthogonal (90°)")
    ax.set_xlabel("Timestep", fontsize=12)
    ax.set_ylabel("Angle (degrees)", fontsize=12)
    ax.set_title(f"Angle between V_base and V_dir (scale={scale:.2f})", fontsize=14)
    ax.set_ylim(0, 180)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    ax.invert_xaxis()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"plot_angles_scale_{scale:.2f}.png"), dpi=150)
    plt.close()
    ...
    ax.set_title(f"Edit Trajectory: Per-step Movement (scale={scale:.2f})", fontsize=14)
    ax.grid(True, alpha=0.3)
    ax.invert_xaxis()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"plot_trajectory_scale_{scale:.2f}.png"), dpi=150)
    plt.close()
...
def plot_vector_comparison(
    ...
    """
    Generate comparison plots across multiple scales.

    Args:
        log_dir: Directory containing stats JSON files
        scales: List of scale values to compare
    ...
    all_data = {}
    for scale in scales:
        stats_path = os.path.join(log_dir, f"stats_scale_{scale:.2f}.json")
        if os.path.exists(stats_path):
            with open(stats_path, "r") as f:
                all_data[scale] = json.load(f)

    if not all_data:
        print(f"No stats files found in {log_dir}")
        return

    # Color map for different scales
    colors = plt.cm.viridis(np.linspace(0, 1, len(scales)))

    # Comparison Plot 1: Norms (V_dir)
    fig, ax = plt.subplots(figsize=(12, 7))
    for (scale, data), color in zip(all_data.items(), colors):
        ax.plot(data["timesteps"], data["V_dir_norm"],
                color=color, linewidth=2, label=f"scale={scale:.1f}")
    ax.set_xlabel("Timestep", fontsize=12)
    ax.set_ylabel("V_dir L2 Norm", fontsize=12)
    ax.set_title("V_dir Norm Comparison across Scales", fontsize=14)
    ...
    fig, ax = plt.subplots(figsize=(12, 7))
    for (scale, data), color in zip(all_data.items(), colors):
        ax.plot(data["timesteps"][1:], data["V_dir_cosine"][1:],
                color=color, linewidth=2, label=f"scale={scale:.1f}")
    ax.axhline(y=0.9, color='gray', linestyle='--', alpha=0.7)
    ax.set_xlabel("Timestep", fontsize=12)
    ax.set_ylabel("Cosine Similarity", fontsize=12)
    ax.set_title("V_dir Directional Consistency Comparison", fontsize=14)
    ax.set_ylim(-0.1, 1.1)
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.invert_xaxis()
    ...
    fig, ax = plt.subplots(figsize=(12, 7))
    for (scale, data), color in zip(all_data.items(), colors):
        ax.plot(data["timesteps"], data["V_base_V_dir_angle"],
                color=color, linewidth=2, label=f"scale={scale:.1f}")
    ax.axhline(y=90, color='gray', linestyle='--', alpha=0.7)
    ax.set_xlabel("Timestep", fontsize=12)
    ax.set_ylabel("Angle (degrees)", fontsize=12)
    ax.set_title("V_base-V_dir Angle Comparison", fontsize=14)
    ax.set_ylim(0, 180)
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.invert_xaxis()
...
def FlowEditFLUX_Slider(
    ...
    """
    FlowEdit with 3-prompt directional decomposition for continuous scale control.
    ...
    Args:
        ...
        tar_prompt_pos: Positive target prompt (e.g., "a severely decayed building")
        tar_prompt_neg: Negative target prompt (e.g., "a new building")
        negative_prompt: Negative prompt for CFG (usually empty for Flux)
        scale: Edit intensity scale (0.0 = tar_neg direction, 1.0 = tar_pos direction)
        T_steps: Total number of timesteps
        n_avg: Number of velocity field averaging iterations
        src_guidance_scale: Guidance scale for source prompt
        tar_guidance_scale: Guidance scale for target prompts
        ...
        scale_mode: Scaling method - "slider" (default), "interp", "step", "cfg", or "direct"
            - "slider": Scale the direction vector V_delta = V_base + scale * V_direction (FreeSlider-like)
            - "interp": FlowEdit-based interpolation V_final = V_src + scale * (V_pos - V_src)
            - "step": Scale the step size dt (FlowEdit paper experiment, causes degradation)
            - "cfg": Scale the target guidance (tar_guidance_scale * scale)
            - "direct": Scale the full velocity difference V_delta = scale * (V_pos - V_src) without decomposition
        normalize_v_dir: If True, normalize V_dir to v_dir_target_norm before scaling.
        ...
        v_dir_target_norm: Target L2 norm for V_dir normalization (default: 1.0).
            Higher values produce stronger edits per unit scale.
    ...
    # Handle guidance
    # ============================================
    # For cfg mode, scale is applied to tar_guidance_scale
    effective_tar_guidance = tar_guidance_scale * scale if scale_mode == "cfg" else tar_guidance_scale
    ...
    # Main editing loop
    # ============================================
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps), desc=f"FlowEdit-Slider (scale={scale:.2f})"):
        ...
        if T_steps - i > n_min:
            ...
            for k in range(n_avg):
                ...
                # ============================================
                # V_direction: Pure edit direction (e.g., "aging" direction)
                V_direction = Vt_pos - Vt_neg

                # V_base: Base change from source to negative target
                V_base = Vt_neg - Vt_src

                # V_delta computation depends on scale_mode
                if scale_mode == "slider":
                    # Slider mode: scale the direction vector (FreeSlider-like)
                    # scale=0 -> V_base only (tar_neg direction)
                    # scale=1 -> V_base + V_direction = Vt_pos - Vt_src (tar_pos direction)

                    # Apply V_dir normalization if enabled
                    if normalize_v_dir:
                        # Compute current norm (mean over sequence dimension)
                        v_dir_norm = V_direction.norm(dim=-1, keepdim=True).mean()
                        # Normalize to target norm
                        V_direction_scaled = V_direction * (v_dir_target_norm / (v_dir_norm + 1e-8))
                    else:
                        V_direction_scaled = V_direction

                    V_delta = V_base + scale * V_direction_scaled
                elif scale_mode == "direct":
                    # Direct mode: scale the full velocity difference without decomposition
                    # V_delta = scale * (V_pos - V_src)
                    # This scales both V_base and V_direction together, causing trajectory collapse at scale > 1
                    V_delta_full = Vt_pos - Vt_src
                    V_delta = scale * V_delta_full
                elif scale_mode == "interp":
                    # Interp mode: FlowEdit-based interpolation
                    # V_final = V_src + scale * (V_pos - V_src) = (1-scale)*V_src + scale*V_pos
                    # For 3-prompt: V_final = V_src + scale * (V_pos - V_neg) + (V_neg - V_src)
                    #                       = V_neg + scale * V_direction
                    # But we want: V_final = V_src + scale * V_delta_full
                    # where V_delta_full = V_pos - V_src (full edit direction)
                    V_delta_full = Vt_pos - Vt_src
                    V_final = Vt_src + scale * V_delta_full
                    # Store V_final directly, will be used differently in ODE propagation
                    V_delta = V_final  # This is actually V_final, not a delta
                elif scale_mode == "step":
                    # Step mode: V_delta is fixed at scale=1, dt will be scaled later
                    V_delta = V_base + V_direction  # equivalent to scale=1
                elif scale_mode == "cfg":
                    # CFG mode: scale was already applied to guidance, use scale=1 for direction
                    V_delta = V_base + V_direction  # equivalent to scale=1
                else:
            ...
            # ============================================
            if log_vectors:
                # Use the last computed V_base and V_direction for logging
                # (when n_avg > 1, this is from the last iteration)
                # Note: For slider mode with normalize_v_dir, log the original V_direction
                ...
                if normalize_v_dir and scale_mode == "slider":
                    ...
                    step_stats["v_dir_original_norm"] = V_direction.norm(dim=-1).mean().item()
                stats_list.append(step_stats)

                # Store current values for next iteration comparison
                prev_V_dir = V_direction.clone()
                prev_zt_edit = zt_edit.clone()

            # Propagate ODE
            zt_edit = zt_edit.to(torch.float32)
            if scale_mode == "step":
                # Step mode: scale the step size dt (FlowEdit paper experiment)
                zt_edit = zt_edit + scale * (t_im1 - t_i) * V_delta_avg
            elif scale_mode == "interp":
                # Interp mode: V_delta_avg is actually V_final, use directly
            ...
            # For final steps, use interpolated target based on scale
            # Interpolate between neg and pos embeddings
            interp_prompt_embeds = (1 - scale) * tar_neg_prompt_embeds + scale * tar_pos_prompt_embeds
            interp_pooled_embeds = (1 - scale) * tar_neg_pooled_prompt_embeds + scale * tar_pos_pooled_prompt_embeds
    ...
    if log_vectors and stats_list:
        stats_path = save_vector_stats(stats_list, log_output_dir, scale)
...
def FlowEditFLUX_Slider_batch(
    ...
    tar_prompt_neg: str,
    negative_prompt: str = "",
    scales: list = [0.0, 0.25, 0.5, 0.75, 1.0],
    T_steps: int = 28,
    n_avg: int = 1,
    src_guidance_scale: float = 1.5,
    tar_guidance_scale: float = 5.5,
...
def FlowEditFLUX_Slider_2prompt(
    ...
    数式: V_delta = scale * (V_tar - V_src)

    scale=0: 編集なし（元画像のまま）
    scale=1: 通常のFlowEdit（tar方向への完全編集）
    scale>1: tar方向への過剰編集
    ...
    Args:
        ...
        scale: Edit intensity (0=no change, 1=full edit)
    """
    device = x_src.device
    orig_height = x_src.shape[2] * pipe.vae_scale_factor
    orig_width = x_src.shape[3] * pipe.vae_scale_factor
    ...
    # Main editing loop
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps), desc=f"FlowEdit-2prompt (scale={scale:.2f})"):
        ...
        if T_steps - i > n_min:
            ...
            for k in range(n_avg):
                ...
                # Simple scaling: V_delta = scale * (V_tar - V_src)
                V_delta = scale * (Vt_tar - Vt_src)
                V_delta_avg += (1 / n_avg) * V_delta
            ...
            # Prompt interpolation for stability at high scales
            # interp = (1 - scale) * src + scale * tar
            interp_prompt_embeds = (1 - scale) * src_prompt_embeds + scale * tar_prompt_embeds
            interp_pooled_embeds = (1 - scale) * src_pooled_prompt_embeds + scale * tar_pooled_prompt_embeds
...
def FlowEditFLUX_Slider_with_mask(
    ...
    # Main editing loop
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps), desc=f"FlowEdit-Slider-Mask (scale={scale:.2f})"):
        ...
        if T_steps - i > n_min:
            ...
            zt_edit = zt_edit.to(torch.float32)
            if scale_mode == "step":
                # Step mode: scale the step size dt
                zt_edit_new = zt_edit + scale * (t_im1 - t_i) * V_delta_avg
            elif scale_mode == "interp":
                # Interp mode: V_delta_avg is actually V_final, use directly
                zt_edit_new = zt_edit + (t_im1 - t_i) * V_delta_avg
            else:
                zt_edit_new = zt_edit + (t_im1 - t_i) * V_delta_avg
...
def FlowEditSD3_Slider(
    ...
    """
    FlowEdit-Slider for SD3 with 3-prompt directional decomposition.
    ...
    Args:
        ...
        n_avg: Number of velocity field averaging iterations
        src_guidance_scale: Guidance scale for source prompt (default: 3.5 for SD3)
        tar_guidance_scale: Guidance scale for target prompts (default: 13.5 for SD3)
        n_min: Number of final steps using regular sampling
        n_max: Maximum number of steps to apply flow editing (default: 33 for SD3)
        scale_mode: Scaling method - "slider" (default), "interp", "step", "cfg", or "direct"
            - "slider": Scale the direction vector V_delta = V_base + scale * V_direction
            - "direct": Scale the full velocity difference V_delta = scale * (V_pos - V_src) without decomposition
        normalize_v_dir: If True, normalize V_dir to v_dir_target_norm before scaling
        v_dir_target_norm: Target L2 norm for V_dir normalization
    ...
    # Main editing loop
    # ============================================
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps), desc=f"FlowEdit-SD3-Slider (scale={scale:.2f})"):
...
def FlowEditZImage_Slider(
    ...
    """
    FlowEdit-Slider for Z-Image with 3-prompt directional decomposition.
    ...
    Args:
        ...
        n_avg: Number of velocity field averaging iterations
        src_guidance_scale: Guidance scale for source prompt (default: 2.0 for Z-Image)
        tar_guidance_scale: Guidance scale for target prompts (default: 6.0 for Z-Image)
        n_min: Number of final steps using regular sampling
    ...
    # Main editing loop
    # ============================================
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps), desc=f"FlowEdit-ZImage-Slider (scale={scale:.2f})"):
        ...
        if T_steps - i > n_min:
            ...
            for k in range(n_avg):
                ...
                V_delta_avg += (1 / n_avg) * V_delta.unsqueeze(0)
            ...
            if log_vectors:
                ...
                if normalize_v_dir and scale_mode == "slider":
                    ...
                    step_stats["v_dir_original_norm"] = V_direction.norm(dim=-1).mean().item()
                stats_list.append(step_stats)
                prev_V_dir = V_direction.unsqueeze(0).clone()
                prev_zt_edit = zt_edit.clone()
            ...
            interp_emb = (1 - scale) * tar_neg_pos_emb + scale * tar_pos_pos_emb
